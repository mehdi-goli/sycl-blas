...
void test_syclblas(int r, char transA, char transB, int m, int n, int k,
  ...
  Executor<SYCL> ex(q);
  {
    typedef vector_view<etype, cl::sycl::buffer<etype, 1> > BufferView;
    buffer<etype, 1> A(dataA.data(), range<1>(dataA.size()));
    buffer<etype, 1> B(dataB.data(), range<1>(dataB.size()));
    buffer<etype, 1> C(dataC.data(), range<1>(dataC.size()));
    auto buffA = BufferView(A);
    auto buffB = BufferView(B);
    auto buffC = BufferView(C);
    run_test(r, 2.0*m*n*k, [&] {
