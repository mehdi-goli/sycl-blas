...
struct benchmark {
  ...
  static std::string align_left(std::string &&text, size_t len, size_t offset=0) {
    return text + std::string((len < text.length() + offset) ? offset : len - text.length(), ' ');
  }

  static void output_headers() {
    std::cout
      << align_left("Test", text_name_length)
      << align_left("Iterations", text_iterations_length)
      << align_left("MFlops", text_flops_length)
    << std::endl;
  }

  static void output_data(const std::string& short_name, int size, int no_reps, int flops, time_units_t dur) {
    double sec = 1e-9*dur.count();
    std::cout
      << align_left(short_name + "_" + std::to_string(size), text_name_length)
      << align_left(std::to_string(no_reps), text_iterations_length)
      << align_left(std::to_string((double(size*flops)/sec)*1e-6), text_flops_length, 1)
      << "MFlops"
    << std::endl;
  }
};
...
#define BENCHMARK_REGISTER_FUNCTION(NAME, FUNCTION, TYPEPARAM)                     \
      for (size_t nelems = step_size; nelems < max_elems; nelems *= step_size) {   \
        const std::string short_name = NAME;                                       \
        auto time = FUNCTION<TYPEPARAM>(num_reps, nelems);                         \
        benchmark<>::output_data(short_name, nelems, num_reps,                     \
            op_flops*1, time);                                                     \
      }
